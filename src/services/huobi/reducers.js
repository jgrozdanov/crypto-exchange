...
export const huobiPriceReducer = (state = initialState.huobi, action) => {
  switch (action.type) {
    case GET_PRICE:
...
export const huobiTradesReducer = (state = initialState.trades.huobi, action) => {
  switch (action.type) {
    case GET_TRADES:
      return { ...state, isLoading: true };
    case GET_TRADES_SUCCESS:
      return { ...state, data: action.data.data, isLoading: false, error: null };
